...
class SAM3DBody(BaseModel):
    ...
    def run_inference_batch(
        ...
        height, width = imgs_list[0].shape[:2]
        cam_int = batch["cam_int"].clone()
        ...
        # Step 2. Re-run with each hand
        ## Left... Flip image & box
        flipped_imgs = [img[:, ::-1] for img in imgs_list]
        tmp = left_xyxy.copy()
        left_xyxy[:, 0] = width - tmp[:, 2] - 1
        left_xyxy[:, 2] = width - tmp[:, 0] - 1

        cam_ints_list = [cam_int[i] for i in range(len(imgs_list))]
        left_xyxy_list = [left_xyxy[i].reshape(1, 4) for i in range(len(imgs_list))]
        batch_lhand = prepare_batch_multi(
            flipped_imgs, transform_hand, left_xyxy_list, cam_ints=cam_ints_list
        )
        batch_lhand = recursive_to(batch_lhand, "cuda")
        lhand_output = self.forward_step(batch_lhand, decoder_type="hand")
        ...
        ## Right...
        right_xyxy_list = [right_xyxy[i].reshape(1, 4) for i in range(len(imgs_list))]
        batch_rhand = prepare_batch_multi(
            imgs_list, transform_hand, right_xyxy_list, cam_ints=cam_ints_list
        )
        batch_rhand = recursive_to(batch_rhand, "cuda")
        rhand_output = self.forward_step(batch_rhand, decoder_type="hand")
